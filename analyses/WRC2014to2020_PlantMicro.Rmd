...
*READ IN THE PLANT FILE (subset 2015, 2018) AND EXP DESIGN* 
```{r Import Files - Plants, include = FALSE}
file_path <- ("../data/WRC_Importance.csv") 
plant <- read.csv(file_path, header=TRUE)
...
otu_file <-("../data/WRC141518.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.pick.opti_mcc.shared")
otu <- read.otu(otu_file)

#READ IN TAXONOMY FILE

tax_file <- ("../data/WRC141518.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.pick.opti_mcc.0.03.cons.taxonomy")
...
#PERMANOVA -- MOWED/UNFERTILIZED AND MOWED/FERTILIZED
#design_bac2 <- cbind(bc_diversity, otu_rel)
...
#Save plot 
ggsave("../figures/plant_evenness.png", plot=plant_shannon, device=NULL, path=NULL, scale=1, width=6.3, height=4.4, dpi=900, limitsize=TRUE)
```
